...
arrow::Status CelebornPartitionWriter::pushPartition(int32_t partitionId) {
  ...
  int32_t commpressedBytesSize = celebornClient_->pushPartitonData(partitionId, dst, size);
  ...
  shuffleWriter_->setPartitionLengths(
      partitionId, shuffleWriter_->partitionLengths()[partitionId] + commpressedBytesSize);
  return arrow::Status::OK();
};
